...
export const createClient=(id,phone,password,sex,name,surname,wilaya,region)=>{
    ...
    return async dispatch =>{
        ...
        try{
            const response= await fetch('http://192.168.1.6:3000/client/addClient',{
                method : "POST",
                headers: {
...
export const setClients= ()=>{
    ...
    return async dispatch =>{
      ...
      try{
           const response= await fetch('http://192.168.1.6:3000/client');
           if(!response.ok){
...
export const setClient= id => {
    return async dispatch=>{
        try{
            const response= await fetch(`http://192.168.1.6:3000/client/${id}`);
            if(!response.ok){
             throw new Error('Oups! Une erreur est survenue.');
             }
...
export const updateClientPassword= (id,password) => {
    ...
    return async dispatch => {
         ...
         try{
           const response = await fetch(`http://192.168.1.6:3000/client/updatePassword/${id}`,{
              method:'PATCH',
              headers: {
                'Content-Type': 'application/json'
            },
...
export const updateClientPhone= (id,phone,clientid) => {
    ...
    return async dispatch => {
         ...
         try{
           const response = await fetch(`http://192.168.1.6:3000/client/updatePhone/${clientid}`,{
              method:'PATCH',
              headers: {
                'Content-Type': 'application/json'
            },
...
export const updateClient= (id,name,surname,email,address,image,wilaya,region) => {
    ...
    return async dispatch => {
         ...
         try{
           const response = await fetch(`http://192.168.1.6:3000/client/updateClient/${id}`,{
              method:'PATCH',
              headers: {
                'Content-Type': 'application/json'
            },
...
export const deleteClient = id => {
    ...
    return async dispatch => {
        ...
        try{
            const response = await fetch(`http://192.168.1.6:3000/client/deleteClient/${id}`,{
               method:'DELETE'});
